...
public final class MatrixResult {
    ...
    public lazy var snapshotDateIntervals: [SnapshotKeyDateIntervalTuple] = MValuationSnapshot.getSnapshotDateIntervals(orderedSnapshots: orderedSnapshots)
    ...
    public lazy var orderedSnapshotKeys: [SnapshotKey] = orderedSnapshots.map(\.primaryKey)

    public lazy var snapshotKeySet: Set<SnapshotKey> = Set(orderedSnapshotKeys)

    public lazy var begSnapshot: MValuationSnapshot? = orderedSnapshots.first

    public lazy var endSnapshot: MValuationSnapshot? = orderedSnapshots.last

    public lazy var begSnapshotKey: SnapshotKey = begSnapshot?.primaryKey ?? MValuationSnapshot.Key.empty

    public lazy var endSnapshotKey: SnapshotKey = endSnapshot?.primaryKey ?? MValuationSnapshot.Key.empty

    public lazy var begCapturedAt: Date = begSnapshot?.capturedAt ?? Date(timeIntervalSinceReferenceDate: 0)

    public lazy var endCapturedAt: Date = endSnapshot?.capturedAt ?? Date(timeIntervalSinceReferenceDate: 0)

    public lazy var capturedAts: [Date] = orderedSnapshots.map(\.capturedAt)
    ...
    public lazy var cashflowItemCount: Int = orderedCashflow.count

    public lazy var hasCashflow: Bool = orderedCashflow.first != nil
    ...
    public lazy var assetKeySet: Set<AssetKey> = Set(positions.map(\.assetKey))
    ...
    private lazy var assetBegPositionsMap: AssetPositionsMap = Dictionary(grouping: begPositions, by: { $0.assetKey })

    private lazy var assetEndPositionsMap: AssetPositionsMap = Dictionary(grouping: endPositions, by: { $0.assetKey })

    private lazy var assetPositionsMap: AssetPositionsMap = Dictionary(grouping: positions, by: { $0.assetKey })

    private lazy var assetCashflowsMap: AssetCashflowsMap = Dictionary(grouping: orderedCashflow, by: { $0.assetKey })

    // MARK: - Accounts

    /// the results of applying the account filter to the available positions for the snapshot
    /// true=retained, false=discarded
    public lazy var accountFilteredMap: AccountFilteredMap = positionsPair.1

    /// participating accounts in the results
    public lazy var orderedAccountKeys: [AccountKey] = accountFilteredMap.filter(\.value).map(\.key).sorted()

    public lazy var accountKeySet: Set<AccountKey> = Set(orderedAccountKeys)

    private lazy var accountBegPositionsMap: AccountPositionsMap = Dictionary(grouping: begPositions, by: { $0.accountKey })

    private lazy var accountEndPositionsMap: AccountPositionsMap = Dictionary(grouping: endPositions, by: { $0.accountKey })

    private lazy var accountPositionsMap: AccountPositionsMap = Dictionary(grouping: positions, by: { $0.accountKey })

    private lazy var accountCashflowsMap: AccountCashflowsMap = Dictionary(grouping: orderedCashflow, by: { $0.accountKey })

    // MARK: - Strategies

    /// participating strategies in the results
    public lazy var orderedStrategyKeys: [StrategyKey] = strategyKeySet.sorted()

    public lazy var strategyKeySet: Set<StrategyKey> = orderedAccountKeys.reduce(into: Set()) { keySet, accountKey in
        let strategyKey = getStrategyKey(accountKey)
        guard strategyKey.isValid else { return }
        keySet.insert(strategyKey)
    }

    private lazy var strategyPositionsMap: StrategyPositionsMap = Dictionary(grouping: positions, by: { getStrategyKey($0.accountKey) })

    private lazy var strategyCashflowsMap: StrategyCashflowsMap = Dictionary(grouping: orderedCashflow, by: { getStrategyKey($0.accountKey) })

    internal func getStrategyKey(_ accountKey: AccountKey) -> StrategyKey {
        accountMap[accountKey]?.strategyKey ?? MStrategy.emptyKey
    }

    // MARK: - Positions

    // positions used to build the matrix values, and the modified dietz
    public lazy var positions: [MValuationPosition] = positionsPair.0

    public lazy var positionsCount: Int = positions.count

    public lazy var positionsMap: SnapshotPositionsMap = Dictionary(grouping: positions, by: { $0.snapshotKey })
    ...
    public lazy var snapshotMarketValueMap: SnapshotValueMap = MatrixResult.getSnapshotMarketValueMap(snapshotKeys: orderedSnapshotKeys,
                                                                                                      snapshotPositionsMap: positionsMap)

    // (negative market value sum ... positive market value sum), by asset and account
    public lazy var assetMarketValueExtentRange: ClosedRange<Double>? = MatrixResult.getAssetMarketValueExtentRange(snapshotKeys: orderedSnapshotKeys,
                                                                                                                    snapshotPositionsMap: positionsMap)

    public lazy var accountMarketValueExtentRange: ClosedRange<Double>? = MatrixResult.getAccountMarketValueExtentRange(snapshotKeys: orderedSnapshotKeys,
                                                                                                                        snapshotPositionsMap: positionsMap)

    public lazy var strategyMarketValueExtentRange: ClosedRange<Double>? = getStrategyMarketValueExtentRange(snapshotKeys: orderedSnapshotKeys,
                                                                                                             snapshotPositionsMap: positionsMap)
    ...
    public lazy var marketValueRange: ClosedRange<Double>? = snapshotMarketValueStats.range
