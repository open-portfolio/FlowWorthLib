...
public final class PendingSnapshot {
    ...
    public lazy var snapshot: MValuationSnapshot = .init(snapshotID: snapshotID, capturedAt: timestamp)

    // MARK: - Accounts

    internal lazy var accountKeys: [AccountKey] = nuPositions.map(\.accountKey)
    ...
    public lazy var nuPositions: [MValuationPosition] = MValuationPosition.createPositions(holdings: netHoldings,
                                                                                           snapshotID: snapshotID,
                                                                                           securityMap: securityMap,
                                                                                           assetMap: assetMap)
        .sorted()

    public lazy var nuMarketValue: Double = nuPositions.reduce(0) { $0 + $1.marketValue }

    // MARK: - Transactions

    /// unfiltered array of transaction keys
    public lazy var transactionKeys: [TransactionKey] = transactions.map(\.primaryKey)
    ...
    public lazy var netTransactions: [MTransaction] = MTransaction.cashflowFilter(transactions: transactions,
    ...
    public lazy var cashflowPeriod: DateInterval? = MValuationCashflow.getCashflowPeriod(begCapturedAt: previousSnapshot?.capturedAt,
    ...
    public lazy var netCashflowTotal: Double = nuCashflows.reduce(0) { $0 + $1.amount }

    internal lazy var netCashflowMap: AccountAssetValueMap = MValuationCashflow.getAccountAssetValueMap(nuCashflows)
