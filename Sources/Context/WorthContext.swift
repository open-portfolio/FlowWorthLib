...
public class WorthContext: BaseContext {
    ...
    public lazy var orderedSnapshots: [MValuationSnapshot] = model.orderedSnapshots
    ...
    public lazy var firstTransactedAt: Date? = orderedTxns.first?.transactedAt

    public lazy var firstSnapshot: MValuationSnapshot? = orderedSnapshots.first

    public lazy var lastSnapshot: MValuationSnapshot? = orderedSnapshots.last

    /// most ancient snapshot capturedAt
    public lazy var firstSnapshotCapturedAt: Date? = firstSnapshot?.capturedAt

    /// most recent snapshot capturedAt
    public lazy var lastSnapshotCapturedAt: Date? = lastSnapshot?.capturedAt

    public lazy var firstSnapshotKey: SnapshotKey? = firstSnapshot?.primaryKey

    public lazy var lastSnapshotKey: SnapshotKey? = lastSnapshot?.primaryKey
    ...
    public lazy var orderedTxns: [MTransaction] = model.orderedTxns

    // MARK: - Valuation Cashflow

    public lazy var orderedCashflow: [MValuationCashflow] = model.orderedCashflowItems
    ...
    public lazy var snapshotPositionsMap: SnapshotPositionsMap = Dictionary(grouping: model.valuationPositions, by: { $0.snapshotKey })
    ...
    public lazy var missingSharePriceTxns: [MTransaction] = orderedTxns.filter { $0.sharePrice == nil }
}
