...
public final class PeriodSummary {
    ...
    public lazy var begMarketValue: Double = begPositions.reduce(0) { $0 + $1.marketValue }

    public lazy var endMarketValue: Double = endPositions.reduce(0) { $0 + $1.marketValue }

    public lazy var deltaMarketValue: Double = endMarketValue - begMarketValue
    ...
    public lazy var daysInPeriod: Double = period.duration / 24 / 60 / 60

    public lazy var yearsInPeriod: Double = daysInPeriod / 365.25
    ...
    public lazy var marketValueDeltaPerYear: Double = marketValueDeltaPerDay * 365.25

    // MARK: - Total Basis

    public lazy var begTotalBasis: Double = begPositions.reduce(0) { $0 + $1.totalBasis }

    public lazy var endTotalBasis: Double = endPositions.reduce(0) { $0 + $1.totalBasis }

    public lazy var deltaTotalBasis: Double = endTotalBasis - begTotalBasis
    ...
    public lazy var accountKeySet: Set<AccountKey> = Set(begAccountMV.keys).union(endAccountMV.keys)
    ...
    public lazy var assetKeySet: Set<AssetKey> = Set(begAssetMV.keys).union(endAssetMV.keys)
    ...
    public lazy var strategyKeySet: Set<StrategyKey> = Set(begStrategyMV.keys).union(endStrategyMV.keys)
    ...
    public lazy var deltaMV: AccountAssetValueMap = endAccountAssetMV.subtract(begAccountAssetMV)
    ...
    public lazy var dietz: MD? = MD(period, dietzMV, dietzCF)
    ...
    internal lazy var dietzMV: MD.MarketValueDelta = .init(start: begMarketValue, end: endMarketValue)
