...
public final class ForecastResult {
    ...
    public lazy var daysInMainPeriod: Double = mainDuration / 24 / 60 / 60

    public lazy var yearsInMainPeriod: Double = daysInMainPeriod / 365.25

    // the NET market values for all the snapshots
    public lazy var mainMarketValues: [Double] = snapshots.map { marketValueMap[$0.primaryKey] ?? 0 }
    ...
    public lazy var mainCapturedAts: [Date] = snapshots.map(\.capturedAt)

    public lazy var maxMV: Double = mainMarketValues.max() ?? 0
    ...
    public lazy var lr: LR? = LinearRegressor(points: mainRelativePoints)

    // MARK: - Projected data

    // formerly horizontalMetrics
    public lazy var hm: ForecastMetrics = .init(mainTimestamps: mainCapturedAts,
    ...
    public lazy var mainResampler: BaseResampler? = AccelLerpResamplerD(hm.mainDistances, targetCount: mainTimeSeriesIndiceCount)
    ...
    public lazy var combinedResampler: BaseResampler? = AccelLerpResamplerD(hm.combinedDistances, targetCount: timeSeriesIndiceCount)
